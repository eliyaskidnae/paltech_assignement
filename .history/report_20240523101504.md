...
### steps
  ...
  2. Adds edges to the graph with the calculated path length as the weight. instead of create a graaph with the whole node i use the `KD-tree` for efficient neighbor searching.
  3.  find an approximate solution to the Traveling Salesman Problem (TSP) starting from a specific waypoint