...
Another approch could be divide the entire working area into a grid of cells. The size of each cell should be at most the dimensions of the robot's working area.then we can idendefiy cells that contains weeds.The robot would start at one end of the grid and move from cell to cell, always keeping the same orientation