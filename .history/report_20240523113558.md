...
### Path Efficiency 
  ...
  1 .**Smooth and Continuous Paths**: By calculating paths based on the robot's turning constraints and the. direction to the next waypoint, the resulting paths are smooth and continuous, avoiding abrupt changes in direction that might be impractical or impossible for the robot to follow.
  2. **Orientation bettewen Way points**:Calculating the angle to the next waypoint based on the slope (i.e., the direction from the current point to the next waypoint) is crucial for ensuring the path efficiency and adheres to these constraints.
