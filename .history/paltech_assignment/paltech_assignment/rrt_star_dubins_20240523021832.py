...
class RRT:
    def __init__(self,svc,k,q,p,area_size ,max_time = 0.1,dubins_radius = 2,is_RRT_star = True):
        ...
        self.dominion  =[0 , area_size , 0 , area_size] 
        self.node_list = {}
        self.max       = max_time 
        self.vertices  =    []
        self.edges     =      [ ]
    ...
    def Rand_Config(self):       
        ...
        x = random.int(self.dominion[0],self.dominion[1])
        y = random.int(self.dominion[2],self.dominion[3])
        qrand = Node(x,y)
...
# 
    ...
    def debiuns_check(self,from_node,to_node):
        '''Check if the path between two nodes is collision free using Dubins path'''
    ...
    def compute_path(self , start , goal):
        ...
        if(self.goal_found):
            return self.reconstract_db_path()
