...
Path Feasibility:
  ...
  1 .*Smooth and Continuous Paths*: By calculating paths based on the robot's turning constraints and the 2 . direction to the next waypoint, the resulting paths are smooth and continuous, avoiding abrupt changes in direction that might be impractical or impossible for the robot to follow.
    Orientation Consideration: Incorporating the angle ensures the robot's orientation is correctly aligned with the path, which is essential for navigation and control.
