...
### Obstacle Avoidance Considertion 
   ...
   1.RRT_DUBINS : this takes the start and the next waypoint and gives us a dubins path considering the obstacle 

   2.State_Validity_Checker: TThehis class, `StateValidityChecker`, is responsible for checking the validity of robot states (individual positions) and paths (sequences of positions) with respect to an occupancy map 
